        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "3rem", width: "100%", paddingTop: "5rem", paddingBottom: "3rem", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 320px", minWidth: 0 }}>
            ...
            <p style={{ opacity: loaded?1:0, transform: loaded?"translateY(0)":"translateY(24px)", transition: "all 0.85s ease 0.46s", fontSize: "clamp(0.93rem,1.8vw,1.08rem)", color: C.textMid, lineHeight: 1.82, margin: "0 0 2rem", fontFamily: "'DM Sans',sans-serif" }}>
              Casa Cavour e un appartamento vacanze con cucina completamente attrezzata nel centro storico di Bertinoro, borgo medievale sulle colline romagnole noto come il <em style={{ color: C.text }}>"Balcone della Romagna"</em> per il panorama sulla Pianura Padana e sulla costa adriatica. Situato in Via Cavour, a pochi passi dalla piazza principale e dalla celebre tradizione del vino della condivisione, e pensato per coppie e viaggiatori che vogliono scoprire la Romagna autentica: cantine, borghi medievali, Sangiovese, Piadina Romagnola. A 15 min da Cesena, 20 min da Forli, 40 min da Rimini, 35 min da San Marino, 60 min da Bologna. Disponibile per soggiorni brevi in italiano, inglese e olandese.
            </p>
...
// Interpreta la sintassi [[etichetta|id-articolo-target]] dentro il testo di
// un paragrafo del blog e la trasforma in link cliccabili verso la pagina
// statica dell'articolo target (stessa sintassi usata da
// generate-blog-pages.mjs per le pagine /post/). Si appoggia allo slugify già
// esistente per calcolare l'URL, così le due implementazioni restano
// sincronizzate senza dover mantenere una mappa separata. Il link si apre
// sempre in una nuova scheda: l'utente non perde il punto in cui si trovava
// nell'articolo che sta leggendo.
...
function renderTestoConLink(testo) {
  ...
  while ((match = INTERNAL_LINK_RE.exec(testo)) !== null) {
    ...
    const href = `https://www.casa-cavour.com/post/${slugify(targetId)}.html`;
    parts.push(
              ...
              <div style={{ maxWidth: 760, margin: "0 auto" }}>
                ...
                {post.contenuto.map((blocco, i) => {
                  if (blocco.tipo === "paragrafo") {
                    return <p key={i} style={{ fontSize: "0.95rem", color: C.textMid, lineHeight: 1.9, fontFamily: "'DM Sans',sans-serif", marginBottom: "1.25rem" }}>{renderTestoConLink(blocco.testo)}</p>;
                  }
...
function Contatti() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const form = e.target;
    if (form.website.value) { setSent(true); return; } // honeypot: bot, finge successo
    const payload = {
  ...
  return (
    <section id="contatti" style={{ background: C.bg2, padding: "7rem 2rem" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <Reveal>
          <div style={{ fontSize: "0.67rem", letterSpacing: "0.28em", color: C.gold, textTransform: "uppercase", fontFamily: "'DM Sans',sans-serif", marginBottom: "1.1rem", display: "flex", alignItems: "center", gap: "0.65rem" }}>
            <span style={{ width: 26, height: 1, background: C.gold, display: "inline-block" }} /> Contatti
          </div>
          <h2 style={{ fontFamily: "'Cormorant Garamond','Playfair Display',serif", fontSize: "clamp(1.8rem,3.5vw,3rem)", color: C.text, fontWeight: 700, lineHeight: 1.1, marginBottom: "1rem", letterSpacing: "-0.02em" }}>
            Una domanda prima<br /><span style={{ color: C.gold, fontStyle: "italic" }}>di prenotare?</span>
          </h2>
          <p style={{ fontSize: "0.94rem", color: C.textMid, lineHeight: 1.85, fontFamily: "'DM Sans',sans-serif", marginBottom: "2.2rem" }}>
            Scrivici per disponibilità, richieste particolari o qualsiasi altra informazione. Rispondiamo di solito entro poche ore.
          </p>
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <input type="text" name="name" placeholder="Nome" required style={inputStyle} />
              <input type="email" name="email" placeholder="Email" required style={inputStyle} />
              <textarea name="message" placeholder="Messaggio" rows={4} required style={{ ...inputStyle, resize: "vertical" }} />
              ...
              {error && (
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.82rem", color: "#b4453a" }}>
                  Non siamo riusciti a inviare il messaggio. Riprova, oppure scrivici su Instagram.
                </div>
              )}
            </form>
          )}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: "3rem", marginBottom: "3rem" }} className="footer-grid">
          ...
          <div>
            ...
            {[["Appartamento","appartamento"],["Posizione","posizione"],["Esperienze","esperienze"],["Testimonianze","testimonianze"],["Blog","blog"],["Domande","qa"],["Contatti","contatti"]].map(([label, anchor]) => (
